...
public class GraphPanel extends JPanel  
{
	...
	// storage variables.
	protected Vector<GraphEntry> logdata = new Vector<GraphEntry>();
	...
	public synchronized void paint(Graphics bg) {
		...
		if (loggerstate == loadinprogress) {
			...
			g.drawString(
					...
					leftBorder, topBorder);
			return;
		} else if (logdata.isEmpty() || minrate >= maxrate
				|| mintime >= maxtime) {
			g.drawString(
					"Could not load any data for graphing.  This may "
							+ "indicate that there was no graphable data inside of the "
							+ "specified log file", leftBorder, topBorder);
			return;
		}
		...
		// long xinterval = (timehi - timelo) / width * (fm.charWidth('A') *
		// 10);
		...
		for (int i = 0; i < width; i += 50) {
			...
			String str = months[month] + " " + date;
			int length = fm.stringWidth(str) / 2;
			g.drawString(str, (leftBorder + i) - length, topBorder + height + 5
					+ string_height);
			str = hours + ":";
			str += String.format("%2d", minutes); // FIXME

			length = fm.stringWidth(str) / 2;
			g.drawString(str, (leftBorder + i) - length, topBorder + height + 5
					+ 2 * string_height);
			g.drawLine((leftBorder + i), topBorder + height, (leftBorder + i),
					topBorder + height + 5);
		}
		...
		for (double r = minrate + yinterval; r < maxrate; r += yinterval) {
			int y = topBorder
					+ (int) ((float) height * (float) (r - minrate) / (float) (maxrate - minrate));
		...
		try {
			...
			while (listiter.hasMoreElements()) {
				...
				int tmpy = topBorder
						+ height
						- (int) ((float) height * (float) (ge.rate - minrate) / (float) (maxrate - minrate));

				if ((tmpx < leftBorder) || (tmpx > width + leftBorder))
					continue;

				// plot the point.
				if (!firstpoint) {
					if ((ge.timestamp - lasttime) > (300 + 1.25 * ge.duration * 10)) {
						// There was a significant lapse in time since the last
						// point,
						// which probably indicates that the client was turned
						// off for
						// awhile, so draw a "drop" in the keyrate graph.
						g.drawLine(lastx, lasty, lastx, topBorder + height);
						g.drawLine(lastx, topBorder + height, tmpx, topBorder
								+ height);
						if (ge.project == 1)
							g.setColor(Color.red); // RC5-64
						else if (ge.project == 2)
						...
						if (ge.project == lastproject) {
							// otherwise just connect the line from the last
							// one.
		...
		string_height /= 2;
		g.setColor(Color.red); // RC5-64
		g.drawLine(leftBorder + 5, topBorder + 3 + (string_height / 2),
				leftBorder + 18, topBorder + 3 + (string_height / 2));
		g.setColor(Color.black);
		g.drawString("RC5-64", leftBorder + 20, topBorder + 3 + string_height);
		g.setColor(Color.yellow); // DES
		g.drawLine(leftBorder + 5, topBorder + 6 + string_height
				+ (string_height / 2), leftBorder + 18, topBorder + 6
				+ string_height + (string_height / 2));
		g.setColor(Color.black);
		g.drawString("DES", leftBorder + 20, topBorder + 6
				+ (string_height * 2));
		g.setColor(Color.green); // CSC
		g.drawLine(leftBorder + 5, topBorder + 9 + (string_height * 2)
				+ (string_height / 2), leftBorder + 18, topBorder + 9
				+ (string_height * 2) + (string_height / 2));
		g.setColor(Color.black);
		g.drawString("CSC", leftBorder + 20, topBorder + 9
				+ (string_height * 3));
		g.setColor(Color.blue); // OGR
		g.drawLine(leftBorder + 5, topBorder + 12 + (string_height * 3)
				+ (string_height / 2), leftBorder + 18, topBorder + 12
				+ (string_height * 3) + (string_height / 2));
		g.setColor(Color.black);
		g.drawString("OGR", leftBorder + 20, topBorder + 12
				+ (string_height * 4));
		g.setColor(Color.magenta); // RC5-72
		g.drawLine(leftBorder + 5, topBorder + 15 + (string_height * 4)
				+ (string_height / 2), leftBorder + 18, topBorder + 15
				+ (string_height * 4) + (string_height / +2));
		g.setColor(Color.black);
		g.drawString("RC5-72", leftBorder + 20, topBorder + 15
				+ (string_height * 5));
		g.setColor(Color.black); // Unknown
		g.drawLine(leftBorder + 75, topBorder + 3 + (string_height / 2),
				leftBorder + 88, topBorder + 3 + (string_height / 2));
		g.setColor(Color.black);
		...
		if (startx > 0) {
			...
			if (startx < endx) {
				if (endx > width + leftBorder)
					endx = width + leftBorder;
				g.fillRect(startx, topBorder, endx - startx, height);
			}
			if (endx < startx) {
				if (startx > width + leftBorder)
					startx = width + leftBorder;
	...
	final class WorkerThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				while (listiter.hasMoreElements()) {
					GraphEntry ge = (GraphEntry) listiter.nextElement();
					if (mintime == 0 || ge.timestamp < mintime)
						mintime = ge.timestamp;
					if (maxtime == 0 || ge.timestamp > maxtime)
						maxtime = ge.timestamp;
					if (minrate == 0.0 || ge.rate < minrate)
						minrate = ge.rate;
					if (maxrate == 0.0 || ge.rate > maxrate)
						maxrate = ge.rate;
			...
			repaint();
			// System.out.println("load complete.  numrecords=" +
			// logdata.size());
		}
	}
