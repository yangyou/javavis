...
class LogParser {
	...
	private Vector<GraphEntry> logdata;
	...
	// ---
	// [May 31 23:24:19 GMT] Completed RC5 block 687C9CC2:40000000 (1073741824
	// keys)
	// 0.00:19:29.52 - [918103.14 keys/sec]
	// ---
	// [Jul 18 03:00:57 GMT] Completed RC5 block 6DE46FD9:00000000 (2147483648
	// keys)
	// [Jul 18 03:00:57 GMT] 0.01:59:18.82 - [299,977.15 keys/sec]
	// ---
	// [Dec 16 03:25:59 UTC] Completed CSC packet 00205AE7:80000000 (4*2^28
	// keys)
	// 0.00:22:46.15 - [786,534.65 keys/sec]
	// ---
	// [Jul 21 10:01:55 UTC] Completed OGR stub 24/2-9-13-29-15 (9,743,881,734
	// nodes)
	...
	// ---
	// [Dec 24 02:43:22 UTC] RC5-72: Completed CA:40749399:00000000 (1.00 stats
	// units)
	// 0.02:49:40.68 - [354,351 keys/s]

	public LogParser(BufferedReader br, Vector<GraphEntry> list) {
		logfile = br;
		logdata = list;
	}
	...
	private int ConvertMonthName(String month) throws ParseException {
	...
	private long ParseTimestamp(String stamp) throws ParseException {
		...
		if (Character.isDigit(sci.first())) {
			...
			if (sci.next() != '/')
				throw new ParseException("bad separator", sci.getIndex());
			...
			if (sci.setIndex(3) != ' ')
				throw new ParseException("bad separator1", sci.getIndex());
			tm_mday = ConvertDecimalInteger(sci);
			if (sci.next() != ' ')
				throw new ParseException("bad separator2", sci.getIndex());
			tm_hour = ConvertDecimalInteger(sci);
			if (sci.next() != ':')
				throw new ParseException("bad separator3", sci.getIndex());
			tm_min = ConvertDecimalInteger(sci);
			if (sci.next() != ':')
				throw new ParseException("bad separator4", sci.getIndex());
			tm_sec = ConvertDecimalInteger(sci);
		...
		if (tm_year < 0)
			throw new ParseException("bad year", sci.getIndex());
		else if (tm_year < 70)
			tm_year += 2000;
		else if (tm_year < 100)
			tm_year += 1900;
		...
		// validate all fields
		if (tm_mon < 1 || tm_mon > 12 || tm_mday < 1 || tm_mday > 31
				|| tm_year < 1970 || tm_year >= 2038 || tm_hour < 0
				|| tm_hour > 23 || tm_min < 0 || tm_min > 59 || tm_sec < 0
				|| tm_sec > 59)
			throw new ParseException("bad field value", sci.getIndex());

		return convertSecondPastEpoc(tm_year, tm_mon, tm_mday, tm_hour, tm_min,
				tm_sec);
	}

	private long convertSecondPastEpoc(int tm_year, int tm_mon, int tm_mday,
			int tm_hour, int tm_min, int tm_sec) {
		// Convert to seconds past epoc.
		// This uses a deprecated API, but it is still useful.
		// return Date.UTC(tm_year - 1900, tm_mon - 1, tm_mday, tm_hour, tm_min,
		// tm_sec) / 100;
		Calendar cal = Calendar.getInstance();
		cal.clear();
		cal.set(tm_year - 1900, tm_mon - 1, tm_mday, tm_hour, tm_min,
				tm_sec);
		return cal.getTime().getTime()/100;
	}
	...
	public float ParseDuration(StringCharacterIterator sci)
		...
		case '.':
			...
			if (sci.next() != ':')
				throw new ParseException("bad duration separator",
						sci.getIndex());
			break;
		default:
		...
		return (float) ((24.0 * days + hours) * (float) 3600.0 + (float) mins
				* (float) 60.0 + (float) secs);
	}

	public int ParseProject(StringCharacterIterator sci) throws ParseException {
		String project = "";
		int projectcode;
		...
		project += sci.next();
		project += sci.next();
		if (project.compareTo("RC5") == 0)
			projectcode = 1;
		else if (project.compareTo("DES") == 0)
			projectcode = 2;
		else if (project.compareTo("CSC") == 0)
			projectcode = 3;
		else if (project.compareTo("OGR") == 0)
			projectcode = 4;
		else if (project.compareTo("72:") == 0)
			projectcode = 8;
	...
	// ge is modified, by reference.
	{
		try {
			...
			if ((ge.timestamp - lastTimeStamp) < -8640000) {
				...
				// System.out.println("Year Roll-Over Time : "+new
				// Date(ge.timestamp*100)+" "+logdata.size());
			...
			if (bContestFirst) {
				// "RC5: Completed"
				ge.project = ParseProject(new StringCharacterIterator(logline1,
						compoffset - 5));
			} else {
				// "Completed RC5"
				ge.project = ParseProject(new StringCharacterIterator(logline1,
			...
			if (keyoffset < 0)
				return false;
			if (bContestFirst) {
				float workfloat = ConvertDecimalFloat(new StringCharacterIterator(
						logline1, keyoffset));
				...
				ge.keycount = (long) ConvertDecimalInteger(new StringCharacterIterator(
						logline1, keyoffset));
			...
			if (rateoffset < 0)
				return false;
			ge.rate = (float) ConvertDecimalFloat(new StringCharacterIterator(
					logline2, rateoffset));
			...
			if (duroffset < 0)
				return false;
			ge.duration = ParseDuration(new StringCharacterIterator(logline2,
					duroffset));
